...
class Attention(Layer):
    ...
    def __init__(self, nb_head, size_per_head, **kwargs):
        ...
        # print('Attention(layer)_nb_head:', nb_head, 'size_per_head:', size_per_head, 'kwargs', **kwargs)
...
def AttentivePooling(dim1, dim2):
    vecs_input = Input(shape=(dim1, dim2), dtype='float32')  # tensor shape=(?,30,400)
    user_vecs = Dropout(0.2)(vecs_input)

    user_att = Dense(200, activation='tanh')(user_vecs)

    user_att = keras.layers.Flatten()(Dense(1)(user_att))  # shape(?,?)

    user_att = Activation('softmax')(user_att)  # shape(?,?)

    user_vec = keras.layers.Dot((1, 1))([user_vecs, user_att])  # shape=(?,400)
...
def get_text_encoder(length, word_embedding_matrix):
    ...
    droped_vecs = Dropout(0.2)(word_vecs)  # shape=(?,30,300)

    word_rep = Attention(20, 20)([droped_vecs] * 3)
    droped_rep = Dropout(0.2)(word_rep)  # shape=(?,?,400)
...
def get_entity_encoder(length, entity_dict):
    ...
    word_rep = Attention(5, 40)([droped_vecs] * 3)  # 5
...
def get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
                     subcategory_dict):
    ...
    title_encoder = get_text_encoder(MAX_TITLE,
                                     title_word_embedding_matrix)
    content_encoder = get_text_encoder(MAX_CONTENT,
...
def news_level_pooling():
    vecs_input = Input(shape=(MAX_TITLE + 2, FASTFORMER_HEAD * 40))
    ...
    title_vec = AttentivePooling(MAX_TITLE, FASTFORMER_HEAD * 40)(title_input)
    vert_vec = Reshape((FASTFORMER_HEAD * 40,))(vert_input)
    subvert_vec = Reshape((FASTFORMER_HEAD * 40,))(subvert_input)
...
def get_user_encoder():
    ...
    user_nc_vecs = Concatenate(axis=-1)([user_vecs, news_vecs])  # (50,400) (500)->(50,)
    user_nc_vecs_dense = Dense(400)(user_nc_vecs)
    user_nc_vecs_dense = Dropout(0.2)(user_nc_vecs_dense)

    user_vecs0 = Attention(TRANSFORMER_HEAD, 20)([user_nc_vecs_dense, user_vecs, user_vecs])
    user_vec = AttentivePooling(MAX_CLICK, TRANSFORMER_HEAD * 20)(user_vecs0)
...
def get_flaten_user_encoder(title_word_embedding_matrix, category_dict, subcategory_dict):
    ...
    print('user_vecs:', user_vecs)

    user_vecs = Fastformer(FASTFORMER_HEAD, 40)([user_nc_vecs, user_vecs, user_vecs])  # 3
    print('user_vecs:', user_vecs)

    user_vecs = Dropout(0.2)(user_vecs)
    print('user_vecs:', user_vecs)

    user_vecs = Reshape((MAX_CLICK, MAX_TITLE + 2, FASTFORMER_HEAD * 40))(user_vecs)  # shape=(?,50,32,120)
    print('user_vecs:', user_vecs)

    user_vecs = TimeDistributed(news_level_pooling())(user_vecs)
    user_vecs = Dropout(0.2)(user_vecs)
...
def create_model_MGCA(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
    ...
    # 候选新闻表示
    contexts_encoder = get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix,
    ...
    user_vec2 = flaten_user_encoder([clicked_title_input, news_vecs])

    user_vecs_all = keras.layers.Concatenate(axis=-1)(
        [user_vec1, user_vec2, user_entity_vecs])  # [(None, 370), (None, 30), (None, 100, 120)]
...
def GraphCoAttNet(num):
    ...
    entity_vecs = Attention(5, 20)([entity_emb] * 3)  # 5
...
def get_entity_graph_encoder(max_entity_num, ):
    ...
    entity_vecs = Attention(2, 50)([entity_input, entity_input, entity_input])  # 2
...
def create_model(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
                 subcategory_dict):
    print('create_model')
    news_encoder = get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict,
                                    category_dict, subcategory_dict)
    user_encoder1 = get_user_encoder()
    flaten_user_encoder = get_flaten_user_encoder(title_word_embedding_matrix, category_dict, subcategory_dict)
    ...
    news_vecs_word = TimeDistributed(news_encoder)(title_inputs)
    news_vecs_word = Dropout(0.2)(news_vecs_word)
    ...
    KG_entity_model = create_model_KG(title_word_embedding_matrix, content_word_embedding_matrix,
                                      entity_dict, category_dict, subcategory_dict)
    user_vec_kg = []
    news_vecs_kg = []
    for i in range(1 + npratio):
        ...
        out = KG_entity_model([ti, ei, eo, clicked_entity_input, clicked_one_hop_input])
        user_entity_vecs = out[0]
        news_vecs_i = out[1]
        user_vec_kg.append(user_entity_vecs)
        news_vecs_kg.append(news_vecs_i)

    user_vec_kg = keras.layers.Concatenate(axis=-2)(user_vec_kg)
    user_vec_kg = keras.layers.Reshape((25000,))(user_vec_kg)
    user_vec_kg = Dense(70)(user_vec_kg)
    user_vec_kg = Dropout(0.2)(user_vec_kg)
    print('user_vec_kg:', user_vec_kg)

    news_vecs = keras.layers.Concatenate(axis=-2)(news_vecs_kg)
    print('news_vecs_kg:', news_vecs)
    user_vec = Concatenate(axis=-1)([user_vec1, user_vec2, user_vec_kg])
    news_vecs = Dense(400)(news_vecs)
    user_encoder = Model([clicked_title_input, title_inputs, clicked_entity_input, clicked_one_hop_input,
                          entity_inputs, one_hop_inputs], user_vec)
...
def create_model_KG(title_word_embedding_matrix, content_word_embedding_matrix, entity_dict, category_dict,
                    subcategory_dict):
    contexts_encoder = get_news_encoder(title_word_embedding_matrix, content_word_embedding_matrix,
                                        entity_dict, category_dict, subcategory_dict)
    title_inputs = Input(shape=(MAX_TITLE + 2 + MAX_CONTENT + MAX_ENTITY,), dtype='int32')
    ...
    news_vecs = keras.layers.Concatenate(axis=-1)([candi_title_vec, news_entity_vecs])
    news_vecs = Reshape((1, 500))(news_vecs)

    model = Model([title_inputs, entity_inputs, one_hop_inputs, clicked_entity_input, clicked_one_hop_input],
                  [user_entity_vecs, news_vecs])
    return model
