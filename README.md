...
1.The example in the case study is not a impression obtained from the preprocessed test set, so you cannot find this impression in the tsv file. Instead, they are constructed by sampling news in MIND dataset. It is reasonably constructed based on actual situations.This approach not only allows us to obtain the best typical examples to illustrate the applicability of the model but also allows for better control of variables.To avoid any misunderstanding, it needs to be clarified here.
